...
describe('Classic, Minimal and Executive print the header gaps (header_spacing_spec.md)', () => {
  ...
  // Name ↔ Title, Photo ↔ Text, Title ↔ Contacts and Icon ↔ Text follow the résumé (D5; 45- to
  // 48-header-*-gap); the rest keep the letterhead's spacing until their rows land.
  it('the cover letter keeps its letterhead\'s spacing whatever other header gaps the résumé sets', async () => {
    const SET = { contactGapX: 30, contactGapY: 12, headerGapBelow: 60, headerRuleGap: 30 };
    for (const template of [...STACKED, 'modern', 'sidebar']) {
