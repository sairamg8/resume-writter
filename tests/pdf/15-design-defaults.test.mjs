...
describe('every per-template table covers every template (VM3-5)', () => {
  ...
  it('the PDF\'s fallbacks, section defaults and components, and the Design panel\'s list, name exactly the templates the app offers', async () => {
    ...
    assert.deepEqual(TEMPLATE_PICKER.map((t) => t.id), ['executive', 'classic', 'modern', 'minimal', 'sidebar', 'timeline', 'banner', 'academic'], 'in its order');
    ...
    assert.deepEqual(
      TEMPLATE_PICKER.filter((t) => t.ats).map((t) => t.id), ['executive', 'classic', 'modern', 'minimal', 'timeline', 'banner', 'academic'],
      'today that is the single-column templates; the Sidebar earns it only in its Single Layout',
    );
    assert.deepEqual(TEMPLATES.toSorted(), ['academic', 'banner', 'classic', 'executive', 'minimal', 'modern', 'sidebar', 'timeline'], 'these tests run every template');
  });
