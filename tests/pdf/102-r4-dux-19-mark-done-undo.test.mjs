...
// in the rank (an epic next to it included), unresolved, and the row is back in its section. A
// repeating issue's next occurrence, made as it was resolved, goes again with the Undo (no duplicate),
// and marking it done once more makes a next one afresh.
...
/** Your work, over `board` in the store, under the kit's ToastProvider; and what a test reads of it. */
async function mountWork(board) {
  ...
  return { view, byLabel, button, toasts, rows, click, issues };
}
...
it('✓ says "<KEY> marked done"; its Undo puts the issue back in its column, at its place in the rank', async () => {
  ...
  const { view, byLabel, button, toasts, rows, click, issues } = await mountWork(board);
  try {
...
it('Undo of a weekly issue takes away its next occurrence, unedited; marking it done again makes one afresh', async () => {
  const board = {
    id: 'p1', key: 'HOME', title: 'Home jobs', color: '#6366f1', mode: 'kanban', columns, labels: [], sprints: [], nextNumber: 3,
    issues: [
      issue('bins', 1, 'Put the bins out', 'c2', { recurrence: 'weekly', recurrenceNextId: null }),
      issue('d', 2, 'Fix the tap', 'c3', { resolvedAt: 1 }),
    ],
  };
  const { view, byLabel, button, click, issues } = await mountWork(board);
  try {
    click(byLabel('Mark HOME-1 done'));
    assert.equal(issues().length, 3, 'the next occurrence is made');
    const nextId = issues().find((i) => i.id === 'bins').recurrenceNextId;
    assert.ok(nextId && issues().some((i) => i.id === nextId));

    click(button('Undo'));
    assert.equal(issues().length, 2, 'Undo leaves no second copy');
    assert.ok(!issues().some((i) => i.id === nextId), 'the next occurrence is gone');
    assert.equal(issues().find((i) => i.id === 'bins').columnId, 'c2');

    click(byLabel('Mark HOME-1 done'));
    assert.equal(issues().length, 3, 'done again: it still repeats');
  } finally {
    await view.unmount();
  }
});
