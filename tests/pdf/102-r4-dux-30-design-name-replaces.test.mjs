...
describe('Save my design under a name already used (R4-DUX-30)', () => {
  ...
  it('the field says a name already used replaces that design, and Save passes its id', async () => {
    ...
    const calls = [];
    const props = { cards, isOn: () => false, onPick: () => {}, saveDesign: (...args) => { calls.push(args); }, engine: 'executive' };
    ...
    try {
      ...
      assert.equal(hint().textContent, 'You already have a design named Clean — saving replaces it on every résumé that uses it.');
  ...
  it('a name taken on another template is refused: replacing it would take the résumés on it off it', async () => {
    const { SavedDesigns } = await loadModule('/src/components/DesignPanelTemplate.jsx');
    const { pickerCards } = await loadModule('/src/utils/templatePicker.js');
    ...
    const cards = pickerCards(cv('resume_a').settings, [design]).filter((c) => c.own);
    const calls = [];
    const props = { cards, isOn: () => false, onPick: () => {}, saveDesign: (...args) => { calls.push(args); }, engine: 'modern' };
    const view = mount(SavedDesigns, props);
    const all = () => [...elements(view.container)];
    const button = (name) => all().find((el) => el.tagName === 'BUTTON' && el.textContent.trim() === name);
    const hint = () => all().find((el) => el.getAttribute('data-testid') === 'design-name-taken');
    try {
      ...
      const input = all().find((el) => el.tagName === 'INPUT');
      view.act(() => reactProps(input).onChange({ target: { value: 'CLEAN' } }));
      assert.ok(hint(), 'the name is taken');
      assert.equal(hint().textContent, `You already have a design named Clean on ${templateLabel('executive')} — choose another name.`);
      assert.equal(reactProps(button('Save')).disabled, true, 'Save is off');
      view.act(() => reactProps(button('Save')).onClick({}));
      view.act(() => reactProps(input).onKeyDown({ key: 'Enter' }));
      assert.deepEqual(calls, [], 'nothing saved, nothing replaced');
    } finally {
      await view.unmount();
    }
  });
