...
/** No text run of the document opens with the separator. */
const noLeadingSeparator = (doc) => assert.ok(!/<w:t(?:\s[^>]*)?>\s*·/.test(doc.xml), `a run opens with " · ": ${JSON.stringify(doc.texts)}`);
...
describe('Word: a project prints its technologies and link on the line under its name, as the PDF does', () => {
  ...
  it('centred: the name, the date, then the technologies and link, each on a line of its own', async () => {
    ...
    assert.equal(lines[0], 'Tidepool');
    assert.ok(lines[1].includes('2021'), JSON.stringify(lines));
    assert.equal(lines[2], `Reactx · ${URL}`);
    noLeadingSeparator(doc);
  });
});
