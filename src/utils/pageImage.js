...
// in react-pdf and pdf.js, which the dashboard does not load at start.
import { renderCoverLetterPdf, renderResumePdf } from './pdfExportReactPDF.js';
...
export async function pageImage(resume, { width = 240, letter = false } = {}) {
  const [blob, pdfjs] = await Promise.all([letter ? renderCoverLetterPdf(resume) : renderResumePdf(resume), loadPdfjs()]);
