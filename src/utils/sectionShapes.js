...
// (R2-110). normalizeResume() runs withSectionShapes() wherever résumés come in, whatever their data
// version.
...
/** The most Grids the PDF lays out (getColumnWidth). */
const MAX_COLUMNS = 4;
...
export function withSectionShapes(r) {
  ...
  const sections = kept.map((s, i) => {
    ...
    const settings = withColumns(s.settings);
