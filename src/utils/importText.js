 ...
 * under the title is too: at the right margin (hint 'end', Executive's "Inline" jobs), or under a line
 * that held both the role and the company (the Timeline's "Side by side" jobs, R2-148).
 */
function readHeader(type, header) {
  ...
  header.forEach((line, k) => {
    ...
    ps.forEach((p, j) => {
      ...
      // A place alone on its line: at the right margin, or under the role and the company.
      if (at < 0 && k > 0 && ps.length === 1 && (line.hint === 'end' || (titled >= 2 && PLACE.test(p) && !ROLE.test(p))) && place(p)) return;
...
function entryOf(type, header, body) {
  ...
  const [p0 = '', p1 = '', ...rest] = type === 'experience' || type === 'volunteering' ? inlinePair(h.parts) : h.parts;
