...
export function PdfPreview({ render, input, zoom = 1, textId, title = 'Résumé', active = true }) {
  ...
  // Back online while the pages print in Noto Sans for a font that could not be loaded, or with no
  // photo for one whose fetch failed: build again, now with that font (R2-146) or photo (R4-LO-18).
  // A build that needs nothing from the web is left alone.
  useEffect(() => {
    const again = () => { if (fontFallback() || imageRetryPending()) setRetry((n) => n + 1); };
