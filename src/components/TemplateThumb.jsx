...
// replaces it. `letter`: the same look's cover-letter letterhead (F1).
 ...
 * `picture`: paint the real page in place of the mock.
 */
export function TemplateThumb({ card, size = 'sm', picture = false, letter = false }) {
  const resume = useMemo(() => lookResume(card, { letter }), [card, letter]);
  const key = `${letter ? 'letter' : 'page'}:${resume.template}:${JSON.stringify(resume.settings)}`;
