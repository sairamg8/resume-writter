...
import { buildRestore, isDemoAccount, needsRestore, originalsIn, rememberCopies } from '@/utils/demoSeed';
...
export function useDemoSeed({ user, appState, store, sync }) {
  ...
  useEffect(() => {
    ...
    if (!ready || seed.restoring || !needsRestore(appState.resumes)) return;
